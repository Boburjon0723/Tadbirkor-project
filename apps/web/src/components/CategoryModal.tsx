...
import { FolderPlus, Tag, Loader2, ArrowRight, ChevronDown, Warehouse, Trash2 } from 'lucide-react';
...
export function CategoryModal({ isOpen, onClose, warehouseId: fixedWarehouseId, warehouseName }: CategoryModalProps) {
  ...
  return (
    <MobileFormShell
      ...
      title={
        <>
          Yangi <span className="text-purple-500">Kategoriya</span>
        </>
      }
      footer={
        <div className="flex items-center gap-3">
          <button
            type="button"
            onClick={onClose}
            className="flex-1 py-3.5 bg-white/5 border border-white/10 rounded-xl text-sm font-black hover:bg-white/10 transition-all"
          >
            Bekor qilish
          </button>
          <button
            ...
            className="flex-[1.5] py-3.5 bg-purple-600 hover:bg-purple-500 text-white font-black rounded-xl transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-2"
          >
